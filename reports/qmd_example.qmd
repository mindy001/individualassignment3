...
We present the standard deviations in @horse-sd-table.

@horse-sd-table

```{python}
#| label: horse-sd-table
...
Additionally, note that in @horse-sd-table we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pop.

![ Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-sd-large width=70%}



In @fig-sd-large we zoom in and look at the province of  `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
\newpage
References